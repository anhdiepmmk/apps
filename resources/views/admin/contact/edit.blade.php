...
@section('content')
	...
	method="post"
	action="{{ URL::to('admin/pages/edit') }}"
	autocomplete="off">
	<input type="hidden" name="_token" value="{{ csrf_token() }}" />
    <input type="hidden" name="id" value="{{$page->id}}" />
        <div class="panel-heading">
            <span class="panel-title"> <i class="panel-title-icon fa fa-plus"></i> Edit Page</span>
        </div>
        <div class="panel-body">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <textarea class="form-control" id="summernote-example" name="noidung">{{ old('noidung', $page->content) }}</textarea>
                </div>
            </div> <!-- / .form-group -->
        </div>
    </form>
