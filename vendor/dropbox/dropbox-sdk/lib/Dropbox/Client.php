...
class Client
{
    ...
    function uploadRemote($url, $basePath)
    {
        Path::checkArg("basePath", $basePath);
        Checker::argStringNonEmpty("query", $url);

        $response = $this->doPost(
            $this->apiHost,
            $this->appendFilePath("1/save_url", $basePath),
            array(
                "url" => $url,
            ));
        return RequestUtil::parseResponseJson($response->body);
    ...
    function uploadRemoteStatus($job_id)
    {
        Checker::argStringNonEmpty("job_id", $job_id);

        $response = $this->doGet(
            $this->apiHost,
            "1/save_url_job/".$job_id
        );
        return RequestUtil::parseResponseJson($response->body);
    }
}
