<?php
    ...
    function icon_view($viewurl){
        return '<a data-container="body" data-original-title="Detail" href="'.$viewurl.'" data-placement="top" data-toggle="tooltip" class="btn btn-xs btn-default add-tooltip"><i class="fa fa-info-circle"></i></a>
        ';
    }

    function icon_edit($editurl){
        return '<a data-container="body" data-original-title="Update" href="'.$editurl.'" data-placement="top" data-toggle="tooltip" class="btn btn-xs btn-default add-tooltip"><i class="fa fa-pencil"></i></a>
        ';
    }

    function icon_del($id){
        return '<a data-container="body" data-original-title="Delete" href="javascript:;" onclick="del('.$id.')" data-placement="top" data-toggle="tooltip" class="btn btn-xs btn-danger add-tooltip delete_record"><i class="fa fa-times"></i></a>';
    }

    function icon_add($url){
        return '<a data-container="body" data-original-title="Create" href="'.$url.'" data-placement="top" data-toggle="tooltip" class="btn btn-xs btn-primary add-tooltip"><i class="fa fa-plus"></i></a>
            ';
    }

    function icon_fix($id){
        return '<a data-container="body" data-original-title="Fix Done" href="javascript:;" onclick="fix('.$id.')" data-placement="top" data-toggle="tooltip" class="btn btn-xs btn-danger add-tooltip delete_record"><i class="fa fa-check"></i></a>';
    }
    function icon_active($table,$field,$id,$status){
        ...
        $html .='<a class="btn btn-xs btn-default add-tooltip" data-container="body" data-toggle="tooltip" data-original-title="'.$title.'" href="javascript:;" onclick="publish('.$table.','.$field.','.$id.','.$status.');"><i class="'.$rep.'"></i></a>';
        $html .='</span>';
        return $html;
    }
    ...
    function evcurl($t, $token, $packagename)
    {
        $url = 'http://api-apk.evozi.com/download';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_HTTPHEADER, ['Host: api-apk.evozi.com',
                                                'Accept: application/json, text/javascript, */*; q=0.01',
                                                'Accept-Language: en-US,en;q=0.5',
                                                'Accept-Encoding: gzip, deflate',
                                                'Content-Type: application/x-www-form-urlencoded; charset=UTF-8',
                                                'Origin: http://apps.evozi.com'
                                                ]);
        curl_setopt($curl, CURLOPT_USERAGENT,'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.10; rv:39.0) Gecko/20100101 Firefox/39.0');
        curl_setopt($curl, CURLOPT_REFERER, 'http://apps.evozi.com/apk-downloader/?id='.$packagename);
        curl_setopt($curl, CURLOPT_POST, 1);
        curl_setopt($curl, CURLOPT_POSTFIELDS, "packagename=.$packagename.&t=.$t.&fdcafffcabbeffb=.$token.&fetch=false");
        $data = curl_exec($curl);
        curl_close($curl);
        return $data;
    }

function _curlpure($url,$post="",$refer="",$usecookie = false) {
    ...
    function _curl_head($url,$post="",$refer="",$usecookie = false) {
    ...
    function postPage($url,$pvars,$timeout = 60){
        if(!isset($timeout))
            $timeout=30;
        $curl = curl_init();
